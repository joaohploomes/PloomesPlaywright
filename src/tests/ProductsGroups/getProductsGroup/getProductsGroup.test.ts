...
test.describe("Get Products Groups", () => {
	...
	test("Get all Products Groups", async () => {
		const productsGroupsController = new ProductsGroupsController();
		const productsGroups = await generateMultipleItens<IProductsGroups>(
			productsGroupsController.createProductGroup.bind(productsGroupsController),
			generateMockedProductsGroups,
			3,
		);
		...
		await deleteMultipleItens<IProductsGroups>(
			productsGroupsController.deleteProductGroup.bind(productsGroupsController),
			productsGroups,
		);
	});
});
